    ...
    var proc = exec('git for-each-ref --format="%(refname) %(upstream)" refs/heads', function (error, stdout, stderr) {
        if (error !== null) {
            ...
            temp.map(function (a) {
                var vars = a.split(" ");
                if (vars[1]){
                    if (envVar.verbose) console.log("found local branch: " + vars[0].replace("refs/heads/", ""));
                    result.push({
    ...
    exec('git ls-remote --heads', function (error, stdout, stderr) {
        if (error !== null) {
            ...
            temp.filter(function (a) {
                var vars = a.split("\t");
                if (envVar.verbose) console.log("found remote branches: " + vars[1].replace("refs/heads/", ""));
                if (vars[1]) {
...
module.exports.main = function () {
    ...
    if (envVar.error) {
        console.log("The parameters sen in were wrong.")
    }
    if (envVar.help) {
